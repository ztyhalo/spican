...
{
//	printk("bus id %s, phydev dev name %s!\n", fixup->bus_id, dev_name(&phydev->dev));
...
{
        int stat2;
//        printk("zty c45 probe devad 0x%x!\n",devad);
        stat2 = mdiobus_c45_read(bus, prtad, devad, MDIO_STAT2);
        if (stat2 < 0)
        {
//        	printk("zty c45 probe read stat2 0x%x!",stat2);
                return stat2;
        }
...
//         if (phy_reg < 0)
//         {
// //        	printk("zty devs in pkg error!\n");
//                 return -EIO;
//         }
//        printk("zty devs in pkg phy reg 0x%x!\n", phy_reg);
//         *devices_in_package = phy_reg << 16;

//         phy_reg = mdiobus_c45_read(bus, addr, dev_addr, MDIO_DEVS1);
//         if (phy_reg < 0)
//         {
// //        	printk("zty devs in pkg error1!\n");
//                 return -EIO;
//         }
//         printk("zty devs in pkg MDIO_DEVS1 phy reg 0x%x!\n", phy_reg);
				   ...
				   u32 *devices_in_package)
{
	...
	*devices_in_package |= (phy_reg & 0xffff);
	...
	printk("zty read device in package 0x%x!\n", *devices_in_package);
...
//                     if (ret < 0)
//                     {
// //                    	printk("zty phy c45 probe present fail!\n");
//                             return -EIO;
//                     }
...
//             if (phy_reg < 0)
//             {
// //            	printk("zty get_phy_c45_devs_in_pkg fail!\n");
//                     return -EIO;
//             }
...
//                             continue;
//             }
// //            printk("zty  c45 read addr 0x%x devaddr 0x%x!\n", addr, i);
//             phy_reg = mdiobus_c45_read(bus, addr, i, MII_PHYSID1);
//             if (phy_reg < 0)
//                     return -EIO;
// //            printk("zty new c45 read phy_reg 0x%x!\n", phy_reg);
...
// //            printk("zty new c45 read c45 id 0x%x!\n", c45_ids->device_ids[i]);
//     }
...
// //	     i < num_ids && c45_ids->devices_in_package == 0;
// //	     i++) {
// //		printk("zty c45 num ids %d!\n", num_ids);
// //		phy_reg = mdiobus_c45_read(bus, addr,1, MII_PHYSID1);
// //
// ////		reg_addr = MII_ADDR_C45 | i << 16 | 6;
// ////		printk("zty c45 read regaddr 0x%x!\n", reg_addr);
// ////		phy_reg = mdiobus_read(bus, addr, reg_addr);
// ////		if (phy_reg < 0)
// ////		{
// ////			printk("zty c45 read phy reg err!\n");
// ////			return -EIO;
// ////		}
// //		printk("zty c45 read phy_reg 0x%x!\n", phy_reg);
// //		c45_ids->devices_in_package = (phy_reg & 0xffff) << 16;
// //
// ////		reg_addr = MII_ADDR_C45 | i << 16 | 5;
// ////		phy_reg = mdiobus_read(bus, addr, reg_addr);
// //		phy_reg = mdiobus_c45_read(bus, addr,1, MII_PHYSID2);
// //		if (phy_reg < 0)
// //		{
// //			printk("zty c45 read phy reg err1!\n");
// //			return -EIO;
// //		}
// //		printk("zty c45 read phy_reg 0x%x!\n", phy_reg);
// //		c45_ids->devices_in_package |= (phy_reg & 0xffff);
...
struct phy_device *get_phy_device(struct mii_bus *bus, int addr, bool is_c45)
{
	...
	if (r)
	{
//		printk("zty get phy id fail!\n");
		return ERR_PTR(r);
	}
//	printk("zty end get phy id 0x%x!\n",phy_id);
	/* If the phy_id is mostly Fs, there is no device there */
	if ((phy_id & 0x1fffffff) == 0x1fffffff)
	{
//		printk("zty get phy id error!\n");
		return NULL;
	}
...
//	printk("phydev register id 0x%x, addr id %d func %pF!\n", phydev->phy_id,
//			phydev->addr,phydev->bus->phy_map[phydev->addr]);
	/* Don't register a phy if one is already registered at this address */
	if (phydev->bus->phy_map[phydev->addr])
		return -EINVAL;
	phydev->bus->phy_mask &=   (~(1 << phydev->addr));
//	printk("zty bus phy mask 0x%x!\n",phydev->bus->phy_mask);
...
//		printk("zty read reset ctl 0x%d!\n", ret);
		if (ret < 0)
			return ret;
...
//			printk("zty phy write err!\n");
	...
	if (ret < 0)
		return ret;
//	printk("zty phy dev drv config init is %pF!\n", phydev->drv->config_init);
...
static inline void read_881_link(struct phy_device *phydev)
{
	int val;
...
{
	int val;
	printk("zty gen10g get features!\n");
	if(phydev->c45_ids.devices_in_package & MDIO_DEVS_AN)
	{
		printk("zty get freature MDIO_DEVS_AN!\n");
		val = mdiobus_c45_read(phydev->bus, phydev->addr, MDIO_MMD_AN, 0x0201);
		...
		printk("zty read mdio ctr1 0x%x!\n", val);

		val = mdiobus_c45_read(phydev->bus, phydev->addr, 1, 0x0834);

		printk("zty read mdio control 0x%x!\n", val);
	}
	val = mdiobus_c45_read(phydev->bus, phydev->addr, MDIO_MMD_PMAPMD,
			        MDIO_STAT2);
	if(val < 0)
	{
		return val;
	}
	printk("zty read pmapmd mdiostat2 0x%x!\n", val);
...
//	printk("zty read 0x931e val 0x%x 0x3863!\n", val);
	zty_c45_write(phydev, 0x01, 0x931D, 0x3710);
