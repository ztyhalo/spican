...
//	printk("zty fec enet start xmit!\n");
	queue = skb_get_queue_mapping(skb);
...
// static int fecRxerrMark = 0;
// static int fecRxcountMark = 0;
static int
...
static void fec_enet_adjust_link(struct net_device *ndev)
{
	...
	if (!netif_running(ndev) || !netif_device_present(ndev)) {
		...
		if (!fep->link) {
//			printk("zty link state fet 0x%x phy 0x%x!\n", fep->link, phy_dev->link);
			fep->link = phy_dev->link;
...
//			printk("zty duplex state fet 0x%x phy 0x%x!\n", fep->full_duplex, phy_dev->duplex);
			fep->full_duplex = phy_dev->duplex;
			status_change = 1;
		}

		if (phy_dev->speed != fep->speed) {
//			printk("zty speed state fet 0x%x phy 0x%x!\n", fep->speed, phy_dev->speed);
			fep->speed = phy_dev->speed;
...
//	printk("zty fec_enet_mii_probe start!\n");

	if (fep->phy_node) {
//		printk("zty get fep phy node!\n");
...
//		printk("zty fec_enet_mii_probe start1!\n");
		for (phy_id = 0; (phy_id < PHY_MAX_ADDR); phy_id++) {
//			printk("zty phy probe %d!\n", dev_id);
//			if(fep->mii_bus)
//			{
//				printk("zty fep mii bus name %s id %d!\n", fep->mii_bus->name,phy_id);
//			}
			if ((fep->mii_bus->phy_mask & (1 << phy_id)))
			{
//				printk("zty fep phy mask 0x%x!\n", fep->mii_bus->phy_mask);
				continue;
			}
			if (fep->mii_bus->phy_map[phy_id] == NULL)
			{
//				printk("zty fep phy mask 0x%x!\n", fep->mii_bus->phy_mask);
//				printk("zty phy map null!\n");
				continue;
			}
			if (fep->mii_bus->phy_map[phy_id]->phy_id == 0)
			{
//				printk("zty phy map phy id  0!\n");
				continue;
			}
			if (dev_id--)
				continue;
			strlcpy(mdio_bus_id, fep->mii_bus->id, MII_BUS_ID_SIZE);
//			printk("zty mdio bus id %s phy id 0x%x!\n", mdio_bus_id,
//					fep->mii_bus->phy_map[phy_id]->phy_id);
			break;
		}
...
static int fec_enet_mii_init(struct platform_device *pdev)
{
	 ...
	 * fec0 mii_bus.
	 */
//	printk("zty fec_enet_mii_init %s!\n",pdev->name);
	...
	if (node) {
//		printk("zty of mdio bus register!\n");
