...
static int mcp251x_spi_direct_trans(struct spi_device *spi, int len)
{
	struct mcp251x_priv *priv = spi_get_drvdata(spi);
	struct spi_transfer t = {
		.tx_buf = priv->spi_tx_buf,
...
static void mcp251x_write_direct_reg(struct spi_device *spi, u8 reg, uint8_t val)
{
	...
	mcp251x_spi_direct_trans(spi, 3);
}

static void mcp251x_write_direct_bits(struct spi_device *spi, u8 reg,
			       u8 mask, uint8_t val)
{
	struct mcp251x_priv *priv = spi_get_drvdata(spi);
...
static struct sk_buff * mcp251x_hw_rx(struct spi_device *spi, int buf_idx)
{
	struct mcp251x_priv *priv = spi_get_drvdata(spi);
	struct sk_buff *skb;
	struct can_frame *frame;
...
static void mcp251x_hw_irq_tx_direct_stage2(struct spi_device *spi, int tx_buf_idx)
{
	struct mcp251x_priv *priv = spi_get_drvdata(spi);
	priv->spi_tx_buf[0] = INSTRUCTION_RTS(1 << tx_buf_idx);
...
{
	mcp251x_write_direct_reg(spi, CANCTRL, CANCTRL_REQOP_SLEEP);
}
...
static int mcp251x_stop(struct net_device *net)
{
	...
	mcp251x_write_direct_reg(spi, CANINTE, 0x00);
	mcp251x_write_direct_reg(spi, CANINTF, 0x00);

	mcp251x_write_direct_reg(spi, TXBCTRL(0), 0);
	mcp251x_clean(net);
...
static irqreturn_t mcp251x_can_ist(int irq, void *dev_id)
{
	...
	for ( i = 0; i < MCP251X_NAPI_WEIGHT; i++)
	{
...
static int mcp251x_irq_process(struct mcp251x_priv *priv)
{
...
static int mcp251x_open(struct net_device *net)
{
	...
	if (!gpio_is_valid(rst_gpio)) {
		printk("hndz mcp251x no rst gpio!\n");
	}
	else
	{
		printk("hndz open mcp251x rst gpio ok!\n");
...
#if MCP2515_MODE == 1
	ret = request_irq(spi->irq, mcp251x_can_ist, flags | IRQF_ONESHOT, DEVICE_NAME, priv);
#elif MCP2515_MODE == 2
	...
	ret = mcp251x_hw_reset(spi);
	if (ret) {
		printk("hndz mcp251x_hw_reset fail!\n");
		mcp251x_open_clean(net);
		goto open_unlock;
	}
	ret = mcp251x_setup(net, priv, spi);
	if (ret) {
		printk("hndz mcp251x_setup fail!\n");
		mcp251x_open_clean(net);
		goto open_unlock;
	}
	ret = mcp251x_set_normal_mode(spi);
	if (ret) {
		printk("hndz mcp251x_set_normal_mode fail!\n");
		mcp251x_open_clean(net);
		goto open_unlock;
	}
...
//	int rst_gpio = 0;
	...
	{
		printk("hndz mcp251x rst gpio ok!\n");
	...
	if (ret)
	{
		printk("hndz mcp251x hw probe error!\n");
		goto error_probe;
	}
	else
		printk("hndz mcp251x hw probe ok!\n");
