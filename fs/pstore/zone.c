		...
		size_t len, unsigned long off)
{
	...
	if (off > zone->buffer_size)
	{
		printk("wwww psz_zone_write dddddddd\n");
		return -EINVAL;
	}
	...
	if (!writeop)
		goto dirty;
	if (zone->type ==PSTORE_TYPE_CONSOLE)
	{
	...
	wcnt = writeop((const char *)zone->buffer, wlen, zone->off);
	return 0;
	if (zone->type ==PSTORE_TYPE_DMESG)
	{
		//wlen = zone->buffer_size + sizeof(*zone->buffer);
		//wcnt = writeop((const char *)zone->buffer, wlen, zone->off);

		gbuf=(char *)zone->buffer;
		wcnt = info->read(xbuf, 5, 0);
		for ( i = 0; i < 5; i++)
			printk("1www xbuf[%d]=%x zone->buffer[%d]=%x\n",i,xbuf[i],i,gbuf[i]);

		//wcnt = writeop((const char *)zone->buffer, wlen, 0);
		wcnt = writeop((const char *)zone->buffer, wlen, 0);
		wcnt = info->read(xbuf, 5, 0);
		for ( i = 0; i < 5; i++)
			printk("2www xbuf[%d]=%x zone->buffer[%d]=%x\n",i,xbuf[i],i,gbuf[i]);

		xbuf[0]=0x46;
		//wcnt = writeop(xbuf, 1, 0);
		wcnt = writeop(xbuf, 1, 0);
		wcnt = info->read(xbuf, 5, 0);
		for ( i = 0; i < 5; i++)
			printk("3www xbuf[%d]=%x zone->buffer[%d]=%x\n",i,xbuf[i],i,gbuf[i]);





		printk("wwww psz_zone_write kkkkkkk zone->off=%llx\n",zone->off);
		wcnt = info->read(xbuf, 10, zone->off);
		for ( i = 0; i < 10; i++)
			printk("www xbuf[%d]=%x zone->buffer[%d]=%x wcnt=%d\n",i,xbuf[i],i,gbuf[i],wcnt);
	}
return 0;
#endif
	switch (flush_mode) {
...
static int psz_kmsg_recover_meta(struct psz_context *cxt)
{
	...
	for (i = 0; i < cxt->kmsg_max_cnt; i++) {
		...
		if (rcnt == -ENOMSG) {
			...
			//printk("wwwxx zone->off=%llx cxt=%llx\n",zone->off,cxt->kpszs[i]->off);
...
static int psz_recover_zone(struct psz_context *cxt, struct pstore_zone *zone)
{
	...
	if (rcnt != len) {
		printk("read zone %s failed\n", zone->name);
		return (int)rcnt < 0 ? (int)rcnt : -EIO;
	}

	if (tmpbuf.sig != zone->buffer->sig) {
		printk("no valid data in zone %s zone->off=%llu\n", zone->name,zone->off);
		printk("tmpbuf.sig=0x%x zone->buffer->sig=0x%x\n", tmpbuf.sig,zone->buffer->sig);
		return 0;
	}
...
static inline int psz_kmsg_erase(struct psz_context *cxt,
		struct pstore_zone *zone, int count)
{
...
static int psz_pstore_erase(enum pstore_type_id type, u64 id, int count,
	...
	//struct pstore_record *record)
{
	...
	case PSTORE_TYPE_DMESG:
		if (id >= cxt->kmsg_max_cnt)
			return -EINVAL;

		return psz_kmsg_erase(cxt, cxt->kpszs[id], count);
	case PSTORE_TYPE_PMSG:
		return psz_record_erase(cxt, cxt->ppsz);
	case PSTORE_TYPE_CONSOLE:
		return psz_record_erase(cxt, cxt->cpsz);
	case PSTORE_TYPE_FTRACE:
		if (id >= cxt->ftrace_max_cnt)
			return -EINVAL;
		return psz_record_erase(cxt, cxt->fpszs[id]);
		...
		///struct pstore_record *record)
{
	...
	hdr->time.tv_sec = 0;//time.tv_sec;
	hdr->time.tv_nsec = 0;//time.tv_nsec;
		...
		//struct pstore_record *record)
{
	...
	for (i = 0; i < cxt->kmsg_max_cnt; i++) {
		...
		printk("wwwwww write %s to zone id %d\n", zone->name, zonenum);
		psz_write_kmsg_hdr(zone, type, reason, id, part,buf,compressed, size,psi);
		hlen = sizeof(struct psz_kmsg_header);
		psize = min_t(size_t, size, zone->buffer_size - hlen);
		ret = psz_zone_write(zone, FLUSH_ALL, buf, psize, hlen);
		...
		printk("zone %u may be broken, try next dmesg zone\n",
				zonenum);
		kfree(zone->buffer);
		zone->buffer = zone->oldbuf;
		zone->oldbuf = NULL;
		...
		//struct pstore_record *record)
{
	...
	ret = psz_kmsg_write_record(cxt, type,reason ,id, part,buf,compressed, size,psi);
	if (!ret && is_on_panic()) {
		/* ensure all data are flushed to storage when panic */
		printk("wwww try to flush other dirty zones\n");
...
//return 0;
	if (!zone)// || !record)
	{
		//printk("wwww psz_record_write zone~\n");
		return -ENOSPC;
	}
	...
	if (unlikely(rem < cnt)) {
		//printk("wwww psz_record_write FLUSH_PART 11\n");
		psz_zone_write(zone, FLUSH_PART, pbuf, rem, start);
		pbuf += rem;
		cnt -= rem;
		start = 0;
		is_full_data = true;
	...
	psz_zone_write(zone, FLUSH_PART, pbuf, cnt, start);
	...
	if (is_full_data) {
		...
		//printk("wwww psz_record_write FLUSH_META\n");
		psz_zone_write(zone, FLUSH_META, NULL, 0, 0);
	}
	return 0;
}

static int notrace psz_pstore_write(enum pstore_type_id type,
					    enum kmsg_dump_reason reason,
					    u64 *id, unsigned int part,
					    const char *buf,
					    bool compressed, size_t size,
	...
	//struct pstore_record *record)
{
	 ...
	 */
	if (is_on_panic() && type != PSTORE_TYPE_DMESG)
		return -EBUSY;

	switch (type) {
	case PSTORE_TYPE_DMESG:
		return psz_kmsg_write(cxt, type, reason, id, part,buf,compressed, size,psi);
	case PSTORE_TYPE_CONSOLE:
		//printk("wwwwwwwwwwwwww\n");
		return psz_record_write(cxt->cpsz, type, reason, id, part,buf,compressed, size,psi);
	case PSTORE_TYPE_PMSG:
		return psz_record_write(cxt->ppsz, type, reason, id, part,buf,compressed, size,psi);
	//case PSTORE_TYPE_FTRACE: {
...
static int psz_kmsg_read_hdr(struct pstore_zone *zone,
				   int *count, struct timespec *time, bool *compressed)
		//struct pstore_record *record)
{
	struct psz_buffer *buffer = zone->buffer;
	struct psz_kmsg_header *hdr =
	...
	*compressed = hdr->compressed;
	time->tv_sec = hdr->time.tv_sec;
	time->tv_nsec = hdr->time.tv_nsec;
	//record->reason = hdr->reason;
	*count = hdr->counter;
	return 0;
}
#if 0
static const char *kmsg_dump_reason_str(enum kmsg_dump_reason reason)
{
	switch (reason) {
...
static ssize_t psz_kmsg_read(struct pstore_zone *zone,
							u64 *id, enum pstore_type_id *type,
		...
		//struct pstore_record *record)
{
	...
	if (psz_kmsg_read_hdr(zone, count, time, compressed)) {
		atomic_set(&zone->buffer->datalen, 0);
...
#if 1
	if (!*compressed) {
		char *pbuf = kasprintf(GFP_KERNEL, "%s: Total %d times\n","Panic",
								/*kmsg_dump_reason_str(record->reason),*/
								*count);
		hlen = strlen(pbuf);
		*buf = krealloc(pbuf, hlen + size, GFP_KERNEL);
		if (!*buf) {
			kfree(pbuf);
			return -ENOMEM;
		}
...
#endif
	//printk("kkkkkk *buf=%p\n", *buf);
	size = psz_zone_read_buffer(zone, *buf + hlen, size,
		...
		//struct pstore_record *record)
{
	...
	ret = pstore_ftrace_combine_log(&(*buf), &record->size,
...
{
	size_t len;
	struct psz_buffer *pbuf;
//return 0;
	if (!zone)// || !record)
		return -ENOSPC;

	pbuf = (struct psz_buffer *)zone->oldbuf;
	if (!pbuf)
		return -ENOMSG;

	len = atomic_read(&pbuf->datalen);
	*buf = kmalloc(len, GFP_KERNEL);
	if (!*buf)
		return -ENOMEM;

	if (unlikely(psz_zone_read_oldbuf(zone, *buf, len, 0))) {
//		printk("wwwwwww kfreekfree *buf=%p\n", *buf);
		kfree(*buf);
		return -ENOMSG;
	}
...
extern ssize_t psblk_generic_blk_read(char *buf, size_t bytes, loff_t pos);
//extern ssize_t psblk_generic_blk_write(const char *buf, size_t bytes,loff_t pos);
//static ssize_t psz_pstore_read(struct pstore_record *record)
static ssize_t psz_pstore_read(u64 *id, enum pstore_type_id *type,
				   int *count, struct timespec *time,
				   char **buf, bool *compressed,
				   struct pstore_info *psi)
{
	...
	char xbuf[512]={0};
	int i;
	//ret=psblk_generic_blk_read(xbuf, 2, 0);
	//printk("wwww111 ret=%d %x %x\n",ret, xbuf[0],xbuf[1]);
	//for ( i = 0; i < 10; i++)
	//	xbuf[i]=i;
	//ret=psblk_generic_blk_write(xbuf, 10, 0);
	memset(xbuf, 0, sizeof(xbuf));
	ret=psblk_generic_blk_read(xbuf, 10, 0);
	for ( i = 0; i < 10; i++)
		printk("wwww222 %x\n", xbuf[i]);
#endif
...
next_zone:
	...
	*type = zone->type;
	...
	ret = readop(zone, id, type,count,time,&(*buf), compressed,psi);
	if (ret == -ENOMSG)
		goto next_zone;
		return ret;
}
		...
		loff_t *off, size_t size)
{
	...
	//{
	//	printk("www init zone->off=%llx\n",zone->off);
	//	printk("www init zone->name=%s\n",zone->name);
	//	printk("www init zone->type=%d\n",zone->type);
	//	printk("www init zone->buffer_size=%zu\n",zone->buffer_size);
	//	printk("www init zone->buffer->sig=%x\n",zone->buffer->sig);
	//}
...
#undef check_size
	...
	pr_cont("\n");
	cxt->pstore.flags &= ~PSTORE_FLAGS_DMESG;
	//printk("wwwwww register_pstore_zone cxt->pstore.flags=%d\n",cxt->pstore.flags);
