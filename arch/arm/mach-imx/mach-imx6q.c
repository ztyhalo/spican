...
/*  1(S)/2  */
#define FEED_WDT_TIME   (HZ/2)
#define FEED_WDT_PIN    IMX_GPIO_NR(7, 8)
...
void feed_wdt(void)
{
	...
	mod_timer(&wdt_timer, jiffies + FEED_WDT_TIME);
	return;
}

static void __init wdt_init(void)
{
	//mxc_iomux_v3_setup_pad(WDT_PIN_MUX);
	gpio_request(FEED_WDT_PIN, "feed watchdog");
	gpio_direction_input(FEED_WDT_PIN);
	Pin_State = !gpio_get_value(FEED_WDT_PIN);
	gpio_direction_output(FEED_WDT_PIN, Pin_State);
	gpio_set_value(FEED_WDT_PIN, 0);
...
static inline u32 zty_c45_addr(int devad, u16 regnum)
{
        return MII_ADDR_C45 | devad << 16 | regnum;
}

static inline void zty_c45_write(struct phy_device *dev, int device, int reg, int val)
{
	phy_write(dev, zty_c45_addr(device,reg), val);
}
...
{
	#define PHY_RESET_PIN	IMX_GPIO_NR(3, 18)
	gpio_request(PHY_RESET_PIN, "phy reset");
	gpio_direction_output(PHY_RESET_PIN, 0);
	msleep(50);
...
#define PHY_ID_BCM881 0xae025032

static void __init imx6q_enet_phy_init(void)
{
	if (IS_BUILTIN(CONFIG_PHYLIB)) {
		// phy_register_fixup_for_uid(PHY_ID_BCM881, MICREL_PHY_ID_MASK,
		// 		bcm881rn_phy_fixup);
...
//	printk("zty find fec fsl,imx6q-fec!\n");
	...
	if (!IS_ERR(gpr))
	{
		...
		if (cpu_is_imx6dl() )
			regmap_update_bits(gpr, IOMUXC_GPR1, BIT(13), BIT(13));
	}
	else
		pr_err("failed to find fsl,imx6q-iomux-gpr regmap\n");
